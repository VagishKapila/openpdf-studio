...
// ===== SIGNATURE REQUEST SIGNERS (multi-signer support) =====
export const signatureRequestSigners = pgTable('signature_request_signers', {
  ...
  signingToken: uuid('signing_token').defaultRandom().notNull().unique(),
  ...
  viewedAt: timestamp('viewed_at'),
  ipAddress: varchar('ip_address', { length: 45 }),
  signedDocS3Key: text('signed_doc_s3_key'),
  createdAt: timestamp('created_at').defaultNow().notNull(),
}, (table) => [
  index('idx_sig_signers_request_id').on(table.requestId),
  index('idx_sig_signers_token').on(table.signingToken),
  index('idx_sig_signers_email').on(table.email),
]);
